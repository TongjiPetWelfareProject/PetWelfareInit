...
namespace Database_CourseDesign
{
    public class ManageSystemInit
    {
        ...
        public void Init(string programPath)
        {
            ...
            InitFunc(managerConnection, userID);
            InitView(managerConnection, userID);
            InitProcedure(managerConnection, userID);
            InitSchedule(managerConnection, userID);
            Console.WriteLine(userID + " 关闭数据库连接");
            //关闭数据库连接
            systemConnection.Close();
        }
        private void InitFunc(OracleConnection connection, string userID)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                try
                {
                    string sql = doc.Descendants("Query").Where(x => (string)x.Attribute("name") == ("CreateFunc" + i.ToString())).FirstOrDefault()?.Value;
                ...
                catch (OracleException ex)
                {
                    if (ex.Code == 955)
                    {
                        Console.WriteLine(i.ToString() + " 已存在同名函数");
                    }
                    else
                    {
        ...
        private void InitProcedure(OracleConnection connection, string userID)
        {
            Console.WriteLine(userID + " 开始创建数据库过程");
            XDocument doc = XDocument.Load(path + "\\SQLXML.xml");
            for (int i = 0; i < 2; i++)
            {
                try
                {
                    string sql = doc.Descendants("Query").Where(x => (string)x.Attribute("name") == ("CreateProcedure" + i.ToString())).FirstOrDefault()?.Value;
                    OracleCommand command = new OracleCommand(sql, connection);
                    command.ExecuteNonQuery();
                }
                catch (OracleException ex)
                {
                    if (ex.Code == 955)
                    {
                        Console.WriteLine(i.ToString() + " 已存在同名过程");
                    }
                    else
                    {
                        Console.WriteLine(i.ToString() + " exception: " + ex.Message);
        ...
        private void InitSchedule(OracleConnection connection, string userID)
        {
            Console.WriteLine(userID + " 开始创建数据库调度程序");
            XDocument doc = XDocument.Load(path + "\\SQLXML.xml");
            for (int i = 0; i < 4; i++)
            {
                try
                {
                    string sql = doc.Descendants("Query").Where(x => (string)x.Attribute("name") == ("CreateSchedule" + i.ToString())).FirstOrDefault()?.Value;
                    OracleCommand command = new OracleCommand(sql, connection);
                    command.ExecuteNonQuery();
                }
                catch (OracleException ex)
